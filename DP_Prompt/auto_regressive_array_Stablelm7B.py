...
class VectorClippedLogitsProcessor(LogitsProcessor):
    def __init__(self, min_vals: Tensor, max_vals: Tensor):
        # min_vals, max_vals: (vocab_size,) tensors on correct device
...
class DPPromptRewriter:
    def __init__(
        ...
        # 1) Load tokenizer & causal LM model
    ...
    def rewrite(self, private_doc: str) -> str:
        ...
        # Causal LM: outputs include the prompt, so slice off input tokens
        gen_tokens = outputs[0, tokenized["input_ids"].shape[-1]:]
...
if __name__ == "__main__":
    rewriter = DPPromptRewriter(
        ...
        min_vals_path="cleaned_yelp_min_logits_Stablelm7B.npy",
        max_vals_path="cleaned_yelp_max_logits_Stablelm7B.npy",
        temperature=0.01,
        max_new_tokens=150,
    )
